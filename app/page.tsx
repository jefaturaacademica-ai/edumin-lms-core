  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-indigo-500 selection:text-white">
      ...
      <header className="max-w-6xl w-full mx-auto px-6 py-6 flex items-center justify-between">
        ...
        <Link href="/login" className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-5 py-2.5 text-sm font-bold text-white transition-all shadow-lg shadow-indigo-600/20">
      ...
      <main className="max-w-6xl w-full mx-auto px-6 py-16 lg:py-24 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="mt-6 text-lg text-slate-400 leading-relaxed max-w-xl">Bienvenido al portal oficial de gestión académica de EDUMIN. Accede a tus cursos, gestiona tus créditos y obtén certificaciones oficiales con validación digital.</p>
          <div className="mt-8 flex flex-col sm:flex-row gap-4">
            <Link href="/login" className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-8 py-4 text-base font-bold text-white transition-all shadow-xl shadow-indigo-600/25">
      ...
      {/* Footer */}
      <footer className="max-w-6xl w-full mx-auto px-6 py-8 border-t border-slate-900 text-center text-sm text-slate-500">&copy; 2026 EDUMIN LMS. Todos los derechos reservados.</footer>
    </div>
  );
}
